...
export default function IncomingCallScreen({ incomingCall, onAccept, onReject, callerInfo }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-[9999] backdrop-blur-sm p-4">
      <div className="bg-gradient-to-b from-gray-900 to-black rounded-2xl p-6 md:p-8 text-center max-w-[420px] w-full mx-auto shadow-2xl border border-gray-700">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <button
            ...
            className={`flex-1 ${isAccepting ? "bg-green-700/80 cursor-wait" : "bg-green-600 hover:bg-green-700"} text-white px-4 py-3.5 rounded-full font-bold transition-all transform hover:scale-105 flex items-center justify-center gap-2 text-base shadow-lg active:scale-95`}
          >
            {isAccepting ? (
          ...
          <button
            onClick={onReject}
            className="flex-1 bg-red-600 hover:bg-red-700 text-white px-4 py-3.5 rounded-full font-bold transition-all transform hover:scale-105 flex items-center justify-center gap-2 text-base shadow-lg active:scale-95"
          >
            <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
